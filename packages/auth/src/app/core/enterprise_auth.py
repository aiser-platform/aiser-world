...
class EnterpriseAuthService:
    ...
    async def authenticate_user(
        ...
        try:
            ...
            if self.config.auth.auto_provision_users and auth_result.user_info:
                local_user = await self._sync_user_to_local_db(
                    auth_result.user_info, db
                )
    ...
    async def _assign_default_organization(
        self, user: User, user_info: UserInfo, db: Session
    ):

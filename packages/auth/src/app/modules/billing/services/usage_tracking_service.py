...
class UsageTrackingService:
        ...
        try:
            ...
            if org:
                ...
                if org.ai_credits_used > org.ai_credits_limit:
                    logger.warning(f"Organization {organization_id} exceeded AI credits limit")
                    return {
                        'success': False,
            ...
            return {
                ...
                'tokens_used': tokens_used,
                'cost_cents': cost_cents,
                'credits_remaining': max(0, org.ai_credits_limit - org.ai_credits_used) if org else 0
            }
    ...
    async def get_usage_stats(
        ...
        try:
            ...
            # Total usage
            total_tokens = base_query.with_entities(func.sum(AIUsageLog.tokens_used)).scalar() or 0
            total_cost_cents = base_query.with_entities(func.sum(AIUsageLog.cost_cents)).scalar() or 0
            ...
            return {
                ...
                'limits': {
                    ...
                    'ai_credits_remaining': max(0, org.ai_credits_limit - org.ai_credits_used) if org else 0,
                    'usage_percentage': (org.ai_credits_used / org.ai_credits_limit * 100) if org and org.ai_credits_limit > 0 else 0
                },
                'breakdown': {
                    'by_model': [
                        {

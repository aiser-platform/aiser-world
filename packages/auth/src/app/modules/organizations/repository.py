...
class OrganizationRepository(
    ...
    async def get_user_organizations(
        ...
        return (
            ...
            .filter(
                UserOrganization.user_id == user_id, UserOrganization.status == "active"
            )
            .all()
        )
    ...
    async def get_usage_stats(self, db: Session, org_id: int) -> Dict[str, Any]:
        ...
        current_month_usage = (
            db.query(func.sum(AIUsageLog.credits_consumed))
            .filter(
        ...
        projects_count = (
            ...
            .filter(Project.organization_id == org_id, not Project.is_deleted)
            .scalar()
            or 0
        )
        ...
        return {
            ...
            "ai_credits_percentage": (current_month_usage / org.ai_credits_limit * 100)
            if org.ai_credits_limit > 0
            else 0,
...
class ProjectRepository(BaseRepository[Project, ProjectCreate, ProjectUpdate]):
    ...
    async def get_by_slug(
        self, db: Session, org_id: int, slug: str
    ) -> Optional[Project]:
    ...
    async def get_user_projects(
        self, db: Session, user_id: int, org_id: Optional[int] = None
    ) -> List[Project]:
        """Get all projects for a user"""
        query = (
            db.query(Project)
            .join(UserProject)
            .filter(UserProject.user_id == user_id, not Project.is_deleted)
        )
    ...
    async def create_with_creator(
        self, db: Session, obj_in: ProjectCreate, creator_id: int
    ) -> Project:
        ...
        user_project = UserProject(
            user_id=creator_id, project_id=project.id, role_id=admin_role.id
        )
        db.add(user_project)
        db.commit()
        db.refresh(project)
...
class RoleRepository(BaseRepository[Role, None, None]):
    ...
    async def get_system_roles(self, db: Session) -> List[Role]:
        ...
        return db.query(Role).filter(Role.is_system_role).all()


class UserOrganizationRepository(
    BaseRepository[UserOrganization, UserOrganizationCreate, None]
):
    def __init__(self):
    ...
    async def get_user_role_in_org(
        ...
        return (
            ...
            .options(joinedload(UserOrganization.role))
            .filter(
    ...
    async def get_organization_members(
        ...
        return (
            db.query(UserOrganization)
            .options(
                joinedload(UserOrganization.user), joinedload(UserOrganization.role)
            )
            .filter(
                UserOrganization.organization_id == org_id,
                UserOrganization.status == "active",
    ...
    async def create_membership(
        self, db: Session, user_id: int, org_id: int, role_name: str
    ) -> UserOrganization:
...
class UserProjectRepository(BaseRepository[UserProject, UserProjectCreate, None]):
    ...
    async def get_user_role_in_project(
        ...
        return (
            ...
            .options(joinedload(UserProject.role))
            .filter(
    ...
    async def get_project_members(
        ...
        return (
            db.query(UserProject)
            .options(joinedload(UserProject.user), joinedload(UserProject.role))
...
class AIUsageLogRepository(BaseRepository[AIUsageLog, AIUsageLogCreate, None]):
    ...
    async def log_usage(self, db: Session, usage_data: AIUsageLogCreate) -> AIUsageLog:
        ...
        org = (
            db.query(Organization)
            .filter(Organization.id == usage_data.organization_id)
            .first()
        )
        if org:
    ...
    async def get_organization_usage(
        self, db: Session, org_id: int, start_date: datetime, end_date: datetime
    ) -> List[AIUsageLog]:
    ...
    async def get_monthly_usage_summary(
        self, db: Session, org_id: int, year: int, month: int
    ) -> Dict[str, Any]:
        ...
        for log in usage_logs:
            operations_count[log.operation_type] = (
                operations_count.get(log.operation_type, 0) + 1
            )
...
class BillingTransactionRepository(
    BaseRepository[BillingTransaction, BillingTransactionCreate, None]
):
    def __init__(self):
        super().__init__(BillingTransaction)

    async def get_organization_transactions(
        self, db: Session, org_id: int, limit: int = 50

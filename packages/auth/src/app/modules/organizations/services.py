...
class OrganizationService(
    BaseService[Organization, None, OrganizationUpdate, OrganizationResponse]
):
    ...
    def get_default_organization(self, db: Session) -> Optional[Organization]:
        ...
        return (
            ...
            .filter(Organization.is_active, not Organization.is_deleted)
            .first()
        )

    def create_organization(
        self, org_data: Dict[str, Any], user_id: int, db: Session
    ) -> OrganizationResponse:
        """
    ...
    def update_organization(
        self, org_id: int, org_data: OrganizationUpdate, user_id: int, db: Session
    ) -> OrganizationResponse:
        ...
        if not user_org:
            raise HTTPException(
                status_code=403,
                detail="Only organization owners can update organization",
            )
    ...
    def check_user_permission(
        self, user_id: int, org_id: int, resource: str, action: str, db: Session
    ) -> bool:
    ...
    def get_organization_dashboard(
        self, org_id: int, user_id: int, db: Session
    ) -> OrganizationDashboard:
        ...
        return OrganizationDashboard(
            organization=OrganizationResponse.from_orm(org), usage_stats=usage_stats
        )
    ...
    def _get_usage_stats(self, org_id: int, db: Session) -> UsageStats:
        ...
        # AI usage
        ai_usage = (
            db.query(AIUsageLog).filter(AIUsageLog.organization_id == org_id).all()
        )
        ...
        project_count = (
            db.query(Project)
            .filter(Project.organization_id == org_id, Project.is_active)
            .count()
        )
...
class PricingService:
    ...
    def upgrade_organization_plan(
        self, org_id: int, plan_type: str, user_id: int, db: Session
    ) -> SubscriptionResponse:
        ...
        if not user_org:
            raise HTTPException(
                status_code=403, detail="Only organization owners can upgrade plans"
            )

        # Get pricing plan
        plan = (
            db.query(PricingPlan)
            .filter(PricingPlan.plan_type == plan_type, PricingPlan.is_active)
            .first()
        )
...
class ProjectService(BaseService[Project, ProjectCreate, None, ProjectResponse]):
    ...
    def create_project(
        self, project_data: ProjectCreate, user_id: int, db: Session
    ) -> ProjectResponse:
        ...
        if not user_org:
            raise HTTPException(
                status_code=403, detail="User must be part of an organization"
            )

...
from app.modules.enterprise.keycloak_integration import (
    keycloak_service,
    EnterpriseAuthMixin,
)
...
class EnterpriseAuthService(EnterpriseAuthMixin):
    ...
    async def _authenticate_with_azure_ad(
        self, db: Session, token: str
    ) -> Optional[Tuple[User, Organization]]:
    ...
    async def _find_or_create_user_from_keycloak(
        self, db: Session, user_info: Dict[str, Any]
    ) -> Optional[User]:
    ...
    async def _get_user_primary_organization(
    ...
    async def _create_default_organization_for_user(
        self, db: Session, user_id: int
    ) -> Organization:
        ...
        org_name = (
            ...
            if user
            else enterprise_config.default_organization_name
        )

        org_data = OrganizationCreate(name=org_name, description="Default organization")

        # Create organization with user as owner
        organization = await self.org_repository.create_with_owner(
            db, org_data, user_id
        )
    ...
    async def provision_enterprise_user(
        ...
        if organization_id:
            ...
            if organization:
                ...
                await self.user_org_repository.create_membership(
                    db, user.id, organization_id, "member"
                )
        else:
    ...
    async def handle_sso_callback(
        self, db: Session, code: str, redirect_uri: str
    ) -> Optional[Tuple[User, Organization, str]]:
    ...
    def is_enterprise_deployment(self) -> bool:
        ...
        return (
            enterprise_config.deployment_mode == DeploymentMode.ON_PREMISE
            or enterprise_config.auth_provider != AuthProvider.INTERNAL
            or enterprise_config.enforce_mfa
        )

...
interface WidgetRendererProps {
    ...
    onConfigUpdate?:
        | ((widgetId: string, config: any) => void)
        | ((config: any) => void);
    // Selection helpers
...
export const WidgetRenderer: React.FC<WidgetRendererProps> = ({
    ...
    if (widget.data && typeof widget.data === 'object') {
        const hasReactElements =
            JSON.stringify(widget.data).includes('"type"') &&
            JSON.stringify(widget.data).includes('"props"');
        if (hasReactElements) {
            console.warn(
                'Widget data contains potential React elements:',
                widget.data
            );
        }
    }
    ...
    if (widget.config && typeof widget.config === 'object') {
        const hasReactElements =
            JSON.stringify(widget.config).includes('"type"') &&
            JSON.stringify(widget.config).includes('"props"');
        if (hasReactElements) {
            console.warn(
                'Widget config contains potential React elements:',
                widget.config
            );
        }
    }
    ...
    const normalizedOnConfigUpdate = (...args: any[]) => {
        ...
        try {
            ...
            try {
                (onConfigUpdate as any)(widget.id, args[0]);
            } catch (err) {
                try {
                    (onConfigUpdate as any)(args[0]);
                } catch (err2) {}
            }
        }
    };
            ...
            case 'text':
                return (
                    <EditableWidget
                        ...
                        onChange={(value) =>
                            normalizedOnConfigUpdate({ content: value })
                        }
    ...
    const widgetStyle: React.CSSProperties = {
        ...
        padding:
            typeof widget.style?.padding === 'number'
                ? widget.style.padding
                : 16,
        backgroundColor:
            widget.style?.backgroundColor || 'var(--ant-color-bg-container)',
            ...
            {widget?.data?.snapshotId && (
                ...
                >
                    <Tag color="blue" style={{ fontSize: 10 }}>
                        Snapshot: {widget.data.snapshotId}
                    </Tag>
                </div>
            )}
            {rendered}
...
}> = ({ widget, onConfigUpdate, onWidgetSelect, selectWidget }) => {
...
const MetricWidgetContent: React.FC<{
    widget: DashboardWidget;
    onConfigUpdate?: (config: any) => void;
    isDarkMode?: boolean;
    onWidgetSelect?: (id: string) => void;
                ...
                <div
                    style={{
                        ...
                        color: trend.startsWith('+')
                            ? 'var(--color-functional-success)'
                            : 'var(--color-functional-error)',
                    }}
                >
                    {trend}
                </div>
...
const FilterWidgetContent: React.FC<{
    widget: DashboardWidget;
    onConfigUpdate?: (config: any) => void;
    onWidgetSelect?: (id: string) => void;
    selectWidget?: (id: string) => void;
...
const DefaultWidgetContent: React.FC<{
    ...
    onWidgetSelect?: (id: string) => void;
    selectWidget?: (id: string) => void;
}> = ({ widget, onWidgetSelect, selectWidget }) => {
    return (
        <div
            style={{
                width: '100%',

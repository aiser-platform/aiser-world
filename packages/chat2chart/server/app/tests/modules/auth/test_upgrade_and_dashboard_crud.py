...
def test_upgrade_and_dashboard_crud():
    ...
    r = s.post(f'{base}/auth/upgrade-demo', json={'demo_token': demo_token, 'user': urllib.parse.quote(json.dumps(user))})
    ...
    create = s.post(f'{base}/charts/dashboards/', json=dash_payload)
    ...
    # Get dashboard
    getd = s.get(f'{base}/charts/dashboards/{dashboard_id}')
    ...
    upd = s.put(f'{base}/charts/dashboards/{dashboard_id}', json=update_payload)
    assert upd.status_code == 200
    upj = upd.json()
    assert upj.get('name') == update_payload['name']

    # Delete dashboard
    delt = s.delete(f'{base}/charts/dashboards/{dashboard_id}')
    assert delt.status_code == 200
    assert delt.json().get('success') is True

    # Confirm deletion
    confirm = s.get(f'{base}/charts/dashboards/{dashboard_id}')

...
from app.modules.projects.models import Project, Organization, OrganizationUser, ProjectDataSource, ProjectConversation
...
class ProjectService(BaseService[Project, ProjectCreate, ProjectUpdate, ProjectResponse]):
        ...
        try:
            async with get_async_session() as db:
                ...
                if not project_data.organization_id:
                    default_org = await self._get_or_create_default_organization(user_id, db)
                    project_data.organization_id = default_org.id
                
                # Check if user can create projects
                can_create = await self._check_user_project_limit(user_id, project_data.organization_id, db)
                if not can_create:
                    raise Exception("User has reached the maximum number of projects allowed")
                
                # Create project
                project_dict = project_data.model_dump()
                project_dict["created_by"] = user_id  # Changed from owner_id to created_by
    ...
    async def _get_or_create_default_organization(self, user_id: str, db) -> Organization:
        """Get or create a default organization for a user"""
        try:
            # Check if user already has a default organization
            query = select(Organization).join(OrganizationUser).where(
    ...
    async def get_project_with_data_sources(self, project_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        """Get project with data source and conversation counts"""
        try:
            async with get_async_session() as db:
                project = await self.repository.get_project_with_data_sources(project_id, user_id, db)
                if not project:
                    return None
                return project
    ...
    async def add_data_source_to_project(self, project_id: str, data_source_id: str, data_source_type: str, user_id: str) -> bool:
        """Add a data source to a project"""
        try:
            async with get_async_session() as db:
                result = await self.repository.add_data_source_to_project(project_id, data_source_id, data_source_type, user_id, db)
                return result
    ...
    async def remove_data_source_from_project(self, project_id: str, data_source_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            success = await self.repository.remove_project_data_source(project_id, data_source_id)
            return success
    ...
    async def get_project_data_sources(self, project_id: str, user_id: str) -> List[Dict[str, Any]]:
        """Get all data sources in a project"""
        try:
            async with get_async_session() as db:
                data_sources = await self.repository.get_project_data_sources(project_id, user_id, db)
                return data_sources
    ...
    async def _check_user_project_access(self, user_id: str, project_id: str, db) -> bool:
        ...
        try:
            ...
            # Owner has access
            if str(project.created_by) == user_id:  # Changed from owner_id to created_by
                return True
    ...
    async def _check_user_project_limit(self, user_id: str, organization_id: str, db) -> bool:
...
class OrganizationService(BaseService[Organization, OrganizationCreate, OrganizationUpdate, OrganizationResponse]):
    ...
    async def get_organization_summary(self, organization_id: str) -> Optional[Dict[str, Any]]:
        """Get organization summary with counts"""
        try:
            async with get_async_session() as db:
                summary = await self.repository.get_organization_summary(organization_id, db)
                return summary

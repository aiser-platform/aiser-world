...
from fastapi import APIRouter, Depends, HTTPException, Response, status  # type: ignore[reportMissingImports]
...
async def update_user_profile(
    user_update: UserUpdate, token: str = Depends(JWTCookieBearer())
):
...
async def update_security_settings(
    settings: dict, token: str = Depends(JWTCookieBearer())
):
...
async def create_api_key(key_data: dict, token: str = Depends(JWTCookieBearer())):
    """Create new API key"""
    try:
...
async def delete_api_key(key_id: str, token: str = Depends(JWTCookieBearer())):
    """Delete API key"""
    try:

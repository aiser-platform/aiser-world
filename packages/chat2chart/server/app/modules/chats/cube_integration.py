...
class CubeIntegrationService:
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                async with session.post(
                    f"{self.ai_analytics_url}/query",
                    json=payload,
                    headers=headers
                ) as response:
                    if response.status == 200:
                        result = await response.json()
                        logger.info(f"✅ AI Analytics query successful for tenant {tenant_id}")
                        return result
                    else:
                        error_text = await response.text()
                        logger.error(f"❌ AI Analytics query failed: {response.status} - {error_text}")
                        return {
                            'success': False,
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                async with session.get(
                    f"{self.ai_analytics_url}/schema/{tenant_id}",
                    headers=headers
                ) as response:
                    if response.status == 200:
                        result = await response.json()
                        ...
                        error_text = await response.text()
                        logger.error(f"❌ Schema fetch failed: {response.status} - {error_text}")
                        return {
                            'success': False,
            ...
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(f"❌ Chart config generation failed: {response.status} - {error_text}")
                        return {
                            'success': False,
    ...
    async def process_chat_query(
        ...
        """
        logger.info(f"🚀 Processing chat query for tenant {tenant_id}: '{natural_language_query}'")
        
        # Step 1: Query AI Analytics
        ai_result = await self.query_ai_analytics(natural_language_query, tenant_id, user_id)
        ...
        return {
            ...
            'chartConfig': chart_result.get('chartConfig') if chart_result.get('success') else None,
    ...
    def classify_query_type(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ['trend', 'over time', 'growth', 'change']):
            ...
            return 'comparisons'
        elif any(word in query_lower for word in ['anomaly', 'unusual', 'spike', 'drop']):
            return 'anomalies'

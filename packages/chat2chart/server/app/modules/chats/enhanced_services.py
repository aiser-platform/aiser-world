...
class EnhancedChatService:
        ...
        try:
            ...
            if self._is_simple_chat(data.prompt):
                ...
                if ai_response.get("success"):
                    message_content = ai_response.get(
            ...
            return ChatResponseSchema(
                conversation=self.__conversation_response, message=message_response
            )
    ...
    async def _process_analytics_query(
        ...
        """
        try:
            logger.info("📊 Processing analytics query via Cube.js")
            ...
            if not cube_result.get("success", False):
                ...
                logger.warning(
                    "⚠️ Cube.js query failed, falling back to regular processing"
                )
    ...
    async def _process_regular_query(self, data: ChatSchema) -> ChatResponseSchema:
        ...
        """
        try:
            logger.info("💬 Processing regular query via Chat2Chart")
    ...
    async def _save_enhanced_conversation(self, enhanced_response: Dict[str, Any]):
        ...
        try:
            ...
            if not conversation:
                conversation = await self._create_enhanced_conversation(
                    enhanced_metadata
                )
            else:
    ...
    async def _save_enhanced_message(
        self, prompt: str, enhanced_response: Dict[str, Any]
    ):
        """
    ...
    async def _ensure_conversation_exists(self):
        ...
        try:
            if not self.__conversation_response:
                ...
                existing = await self.__conversation_repository.get_by_id(
                    self.__conversation_id
                )
                if existing:
                    ...
                    self.__conversation_response = (
                        await self.__conversation_repository.create(conversation_data)
                    )
        except Exception as e:
    ...
    def _is_simple_chat(self, prompt: str) -> bool:
        ...
        return (
            any(pattern in prompt_lower for pattern in simple_patterns)
            or len(prompt_lower) < 10
        )
    ...
    async def _ensure_conversation_exists(self):
        ...
        try:
            if not self.__conversation_response:
                # Try to get existing conversation
                existing_conv = await self.__conversation_repository.get_by_id(
                    self.__conversation_id
                )
                ...
                if existing_conv:
                    ...
                    self.__conversation_response = (
                        await self.__conversation_repository.create(conv_data)
                    )
        except Exception as e:
            logger.error(f"Error ensuring conversation exists: {e}")
            # Create a minimal response

...
class SampleDataService:
    ...
    def _generate_bank_data(self) -> Dict[str, Any]:
        ...
        account_types = np.random.choice(['Savings', 'Checking', 'Premium', 'Student'], n_customers, p=[0.4, 0.35, 0.2, 0.05])
        ...
        for i in range(n_customers):
            n_transactions = np.random.poisson(15)  # Average 15 transactions per customer
            
            for _ in range(n_transactions):
                transaction_date = datetime.now() - timedelta(days=np.random.randint(1, 365))
                amount = np.random.normal(0, 100)
                transaction_type = np.random.choice(['Deposit', 'Withdrawal', 'Transfer', 'Payment'], p=[0.3, 0.25, 0.25, 0.2])
        ...
        for i in range(n_customers):
            customers.append({
                ...
                'balance': balances[i],
                'join_date': (datetime.now() - timedelta(days=np.random.randint(100, 2000))).strftime('%Y-%m-%d'),
    ...
    def _generate_ecommerce_data(self) -> Dict[str, Any]:
        ...
        for _ in range(n_sales):
            ...
            sales.append({
                ...
                'customer_segment': np.random.choice(['New', 'Returning', 'VIP'], p=[0.4, 0.45, 0.15]),
    ...
    def _generate_telecom_data(self) -> Dict[str, Any]:
        ...
        for i in range(n_customers):
            ...
            # Usage patterns
            data_usage = np.random.normal(plan_details['data_gb'] * 0.8, plan_details['data_gb'] * 0.3)
            data_usage = max(0, data_usage)
            
            minutes_used = np.random.normal(plan_details['minutes'] * 0.7, plan_details['minutes'] * 0.4)
            ...
            customers.append({
                ...
                'contract_length_months': np.random.choice([12, 24, 36], p=[0.4, 0.4, 0.2]),
                'is_churned': np.random.choice([True, False], p=[0.15, 0.85]),
                'join_date': (datetime.now() - timedelta(days=np.random.randint(100, 1500))).strftime('%Y-%m-%d')
            })
        ...
        return {
            ...
            'metadata': {
                'total_customers': n_customers,
                'churn_rate': round(sum(1 for c in customers if c['is_churned']) / len(customers) * 100, 2),
                'avg_satisfaction': round(sum(c['satisfaction_score'] for c in customers) / len(customers), 2),
                'avg_monthly_revenue': round(sum(c['monthly_bill'] for c in customers) / len(customers), 2),
                'currency': 'USD'
            }
        }
    ...
    def _generate_poverty_data(self) -> Dict[str, Any]:
        ...
        for i in range(n_regions):
            ...
            healthcare_access = round(100 - poverty_rate * np.random.uniform(0.3, 0.8), 2)
        ...
        for region_data in regional_data:
            ...
            for age_group in age_groups:
                ...
                demographics.append({
                    ...
                    'population_count': int(region_data['population'] * np.random.uniform(0.1, 0.2)),
                    'education_attainment': round(100 - poverty_rate * np.random.uniform(0.5, 1.0), 2)
                })
        ...
        return {
            ...
            'metadata': {
                ...
                'avg_poverty_rate': round(sum(r['poverty_rate'] for r in regional_data) / len(regional_data), 2),
    ...
    def _generate_dataset_summary(self, dataset_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if dataset_type == 'bank_customers':
            ...
            return {
                ...
                'total_transactions': len(transactions),
                'avg_balance': round(sum(c['balance'] for c in customers) / len(customers), 2),
                'avg_age': round(sum(c['age'] for c in customers) / len(customers), 1),
                'account_type_distribution': {acc: sum(1 for c in customers if c['account_type'] == acc) for acc in set(c['account_type'] for c in customers)},
            ...
            return {
                ...
                'avg_order_value': round(sum(s['total_amount'] for s in sales) / len(sales), 2),
                'category_distribution': {cat: sum(1 for s in sales if s['category'] == cat) for cat in set(s['category'] for s in sales)},
            ...
            return {
                ...
                'avg_satisfaction': round(sum(c['satisfaction_score'] for c in customers) / len(customers), 2),
                'avg_monthly_revenue': round(sum(c['monthly_bill'] for c in customers) / len(customers), 2),
                'plan_distribution': {plan: sum(1 for c in customers if c['plan_type'] == plan) for plan in set(c['plan_type'] for c in customers)},
                'avg_data_usage': round(sum(c['data_usage_gb'] for c in customers) / len(customers), 2)
            }
            ...
            return {
                ...
                'avg_poverty_rate': round(sum(r['poverty_rate'] for r in regional_data) / len(regional_data), 2),
                'total_population': sum(r['population'] for r in regional_data),
                'region_distribution': {reg: sum(1 for r in regional_data if r['region_type'] == reg) for reg in set(r['region_type'] for r in regional_data)},
                'area_type_distribution': {area: sum(1 for r in regional_data if r['area_type'] == area) for area in set(r['area_type'] for r in regional_data)},
                'avg_median_income': round(sum(r['median_income'] for r in regional_data) / len(regional_data), 2)
            }
    ...
    def _generate_bank_age_chart(self) -> Dict[str, Any]:
        ...
        return {
            "title": {"text": "Customer Age Distribution by Account Type", "left": "center"},
            "tooltip": {"trigger": "axis"},
            "legend": {"top": "30px"},
            "xAxis": {"type": "category", "data": ["18-25", "26-35", "36-45", "46-55", "56-65", "65+"]},
    ...
    def _generate_ecommerce_category_chart(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tooltip": {"trigger": "axis"},
            "xAxis": {"type": "category", "data": ["Electronics", "Clothing", "Home & Garden", "Books", "Sports", "Beauty", "Toys", "Automotive"]},
    ...
    def _generate_ecommerce_trends_chart(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tooltip": {"trigger": "axis"},
            "xAxis": {"type": "category", "data": ["Day 1", "Day 5", "Day 10", "Day 15", "Day 20", "Day 25", "Day 30"]},
    ...
    def _generate_ecommerce_price_rating_chart(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "series": [{
                ...
                "type": "scatter",
                "data": [
                    [25, 4.2], [45, 4.5], [120, 4.8], [89, 4.1], [67, 4.6],
                    [156, 4.9], [34, 3.8], [78, 4.3], [200, 4.7], [95, 4.4]
                ],
    ...
    def _generate_telecom_satisfaction_chart(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tooltip": {"trigger": "axis"},
            "xAxis": {"type": "category", "data": ["Basic", "Standard", "Premium", "Unlimited"]},
    ...
    def _generate_poverty_education_chart(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "series": [{
                ...
                "type": "scatter",
                "data": [
                    [85, 8.5], [72, 15.2], [78, 12.8], [82, 9.1], [75, 13.6],
                    [68, 18.2], [79, 11.9], [81, 10.3], [76, 14.1], [83, 8.9]
                ],

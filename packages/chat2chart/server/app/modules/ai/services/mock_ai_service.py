...
class MockAIService:
    ...
    async def generate_intelligent_response(
        ...
        try:
            ...
            if any(
                word in prompt_lower for word in ["database", "sql", "table", "query"]
            ):
                ...
                word in prompt_lower for word in ["analyze", "data", "trend", "insight"]
            ):
            ...
            if category == "chart_requests":
                ...
                base_response = base_response.format(
                    chart_type=chart_type, insight=insight
                )
            ...
            if context and context.get("data_source_connected"):
                enhancements.append(
                    "I can see you have a data source connected, which gives me access to your actual data for analysis."
                )

            if context and context.get("conversation_history"):
                enhancements.append(
                    "Based on our previous conversation, I'll continue building on the insights we've discovered."
                )

            if any(word in prompt_lower for word in ["ebi", "database", "postgres"]):
                enhancements.append(
                    "I notice you're working with the EBI PostgreSQL database. This is a rich scientific dataset perfect for analysis."
                )
    ...
    def _generate_next_steps(
        self, prompt_lower: str, context: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    async def analyze_query_intent(self, query: str) -> Dict[str, Any]:
        ...
        if any(
            ...
            intent_analysis["suggested_actions"].append(
        ...
        if any(
            word in query_lower
            for word in ["analyze", "data", "database", "table", "chart"]
        ):
            intent_analysis["requires_data"] = True
            intent_analysis["suggested_actions"].append(
                "Connect a data source to enable analysis"
            )
        ...
        complexity_indicators = len(
            [
                word
                for word in ["analyze", "compare", "predict", "forecast", "correlation"]
    ...
    def get_chart_recommendation(
        self, query: str, data_context: Optional[Dict] = None
    ) -> Dict[str, Any]:
        ...
        if any(word in query_lower for word in ["trend", "over time", "growth"]):
            ...
            word in query_lower for word in ["distribution", "breakdown", "proportion"]
        ):
            return {

...
class AIOrchestrator:
    ...
    async def orchestrate_analysis(
        ...
        try:
            ...
            data_understanding = await self._data_understanding_agent(query, data_context)
            
            # Phase 2: Analysis Planning Agent
            analysis_plan = await self._analysis_planning_agent(query, data_context, data_understanding)
            
            # Phase 3: SQL Generation Agent (if applicable)
            sql_queries = []
            if data_context.get('type') in ['database', 'warehouse', 'cube']:
                sql_queries = await self._sql_generation_agent(query, data_context, analysis_plan)
            
            # Phase 4: Chart Recommendation Agent
            chart_recommendations = await self._chart_recommendation_agent(query, data_context, analysis_plan)
            
            # Phase 5: Insight Generation Agent
            insights = await self._insight_generation_agent(query, data_context, analysis_plan, sql_queries)
            
            # Phase 6: Business Context Agent
            business_context = await self._business_context_agent(query, data_context, insights)
            
            # Phase 7: Final Synthesis
            final_response = await self._synthesize_response(
                query, data_context, analysis_plan, sql_queries, 
                chart_recommendations, insights, business_context
            )
    ...
    async def analyze_data_with_context(
        ...
        """
        try:
            logger.info(f"🚀 Starting AI Orchestrator analysis for query: {user_query[:100]}...")
            ...
            logger.info(f"📋 Context built successfully: {len(context.get('cube_js_sources', []))} cube sources, {len(context.get('database_sources', []))} database sources, {len(context.get('file_sources', []))} file sources")
            ...
            analysis_strategy = self._determine_analysis_strategy(data_sources, user_query, user_context)
            ...
            enhanced_prompt = self._build_enhanced_prompt(
                user_query, context, analysis_strategy, conversation_history, user_context
            )
            ...
            if analysis_strategy == "cube_js":
                result = await self._execute_cube_analysis(user_query, context, enhanced_prompt)
            elif analysis_strategy == "database":
                result = await self._execute_database_analysis(user_query, context, enhanced_prompt)
            else:
                result = await self._execute_file_analysis(user_query, context, enhanced_prompt)
            
            logger.info(f"✅ Analysis execution completed: {result.get('type', 'unknown')}")
    ...
    async def _build_data_context(self, data_sources: List[Dict]) -> Dict:
        ...
        for source in data_sources:
            ...
            logger.info(f"📊 Processing {source_type} source: {source_name} ({source_id})")
            ...
            if source_type == "cube":
                ...
                context["available_dimensions"].extend(cube_context.get("dimensions", []))
                ...
                logger.info(f"📁 Added file source to context: {len(context['file_sources'])} total")
            else:
                logger.warning(f"⚠️ Unknown source type: {source_type} for source {source_name}")
        ...
        logger.info(f"✅ Data context built: {len(context['cube_js_sources'])} cube, {len(context['database_sources'])} database, {len(context['file_sources'])} file sources")
    ...
    def _get_source_capabilities(self, source: Dict) -> Dict:
        ...
        if source_type == "cube":
            capabilities.update({
                ...
                "can_join": True,
                "supported_operations": ["semantic_queries", "pre_aggregations", "time_granularity", "business_logic"]
            })
        elif source_type == "database":
            capabilities.update({
                "can_aggregate": True,
                ...
                "can_time_series": True,
                "can_join": True,
                "supported_operations": ["sql_queries", "complex_joins", "stored_procedures", "views"]
            })
        elif source_type == "file":
            capabilities.update({
                "can_aggregate": True,
                ...
                "can_join": False,
                "supported_operations": ["column_analysis", "statistical_summary", "pattern_recognition"]
            })
    ...
    def _determine_analysis_strategy(self, data_sources: List[Dict], user_query: str = "", user_selection: Dict = None) -> str:
        ...
        if user_selection and user_selection.get("selected_data_source_id"):
            ...
            selected_source = next((ds for ds in data_sources if ds["id"] == selected_id), None)
            if selected_source:
                logger.info(f"🎯 User explicitly selected {selected_source['type']} source: {selected_source['name']}")
        ...
        if active_sources:
            ...
            if query_intent.get("prefers_cube") and any(s["type"] == "cube" for s in active_sources):
                return "cube_js_intent_driven"
            elif query_intent.get("prefers_database") and any(s["type"] == "database" for s in active_sources):
                return "database_intent_driven"
            elif query_intent.get("prefers_file") and any(s["type"] == "file" for s in active_sources):
    ...
    def _analyze_user_intent(self, user_query: str) -> Dict:
        ...
        cube_keywords = [
            "semantic", "business logic", "pre-aggregated", "measures", "dimensions",
            "cube", "analytics", "business intelligence", "kpi", "dashboard"
        ]
        ...
        database_keywords = [
            "sql", "query", "join", "table", "schema", "database", "warehouse",
            "complex", "advanced", "technical", "raw data", "custom logic"
        ]
        ...
        file_keywords = [
            "upload", "file", "csv", "excel", "spreadsheet", "simple", "quick",
        ...
        if any(word in query_lower for word in ["trend", "time", "over time", "growth", "forecast"]):
            intent["analysis_type"] = "time_series"
            intent["time_sensitive"] = True
        elif any(word in query_lower for word in ["compare", "vs", "versus", "difference", "ranking"]):
            intent["analysis_type"] = "comparative"
        elif any(word in query_lower for word in ["correlation", "relationship", "pattern", "insight"]):
    ...
    def _get_active_data_sources(self, data_sources: List[Dict], user_selection: Dict = None) -> List[Dict]:
        ...
        if user_selection:
            ...
            if selected_id:
                ...
                if selected_source:
                    ...
                    logger.info(f"🎯 User selected active source: {selected_source['name']} ({selected_source['type']})")
        ...
        if user_selection and user_selection.get("active_data_sources"):
            ...
            for active_id in active_ids:
                source = next((ds for ds in data_sources if ds["id"] == active_id), None)
                if source and source not in active_sources:
                    active_sources.append(source)
                    logger.info(f"📊 Active source from panel: {source['name']} ({source['type']})")
        ...
        if not active_sources:
            ...
            if cube_sources:
                ...
                logger.info(f"🔄 Defaulting to primary Cube.js source: {cube_sources[0]['name']}")
            else:
                # Fall back to first available source
                if data_sources:
                    active_sources.append(data_sources[0])
                    logger.info(f"🔄 Defaulting to first available source: {data_sources[0]['name']} ({data_sources[0]['type']})")
    ...
    def _build_enhanced_prompt(
        ...
        context_instructions = self._build_context_specific_instructions(context, strategy, user_selection)
...
"""
    ...
    def _build_intent_context(self, user_intent: Dict, strategy: str) -> str:
        ...
        intent_context.append(f"Analysis type: {user_intent.get('analysis_type', 'general')}")
        intent_context.append(f"Complexity level: {user_intent.get('complexity_level', 'basic')}")
        ...
        return "; ".join(intent_context) if intent_context else "General analysis requested"

    def _build_context_specific_instructions(self, context: Dict, strategy: str, user_selection: Dict = None) -> str:
        ...
        if user_selection and user_selection.get("selected_data_source_id"):
            selected_id = user_selection["selected_data_source_id"]
            instructions.append(f"IMPORTANT: User has specifically selected data source {selected_id} for analysis.")
            instructions.append("Prioritize this selection and provide analysis based on this source.")
        ...
        if "cube" in strategy.lower():
            ...
            if context.get("cube_js_sources"):
                ...
                instructions.append(f"- Available measures: {len(cube_source.get('measures', []))}")
                instructions.append(f"- Available dimensions: {len(cube_source.get('dimensions', []))}")
                instructions.append(f"- Pre-aggregations: {len(cube_source.get('pre_aggregations', []))}")
            ...
            if context.get("database_sources"):
                ...
                instructions.append(f"- Available tables: {len(db_source.get('tables', {}))}")
                instructions.append(f"- Database type: {db_source.get('type', 'unknown')}")
            ...
            if context.get("file_sources"):
                ...
                instructions.append(f"- File type: {file_source.get('file_type', 'unknown')}")
                instructions.append(f"- Total rows: {file_source.get('row_count', 'unknown')}")
    ...
    async def _execute_cube_analysis(self, query: str, context: Dict, prompt: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "visualization_config": self._generate_echarts_config(enhanced_result, query),
    ...
    async def _execute_database_analysis(self, query: str, context: Dict, prompt: str) -> Dict:
    ...
    async def _execute_file_analysis(self, query: str, context: Dict, prompt: str) -> Dict:
        ...
        try:
            ...
            if not file_data:
                raise Exception(f"No data available for analysis in {file_source['name']}")
            
            # Analyze the actual data
            analysis_result = await self._analyze_file_data(query, file_data, file_source)
            
            # Generate chart configuration based on analysis
            chart_config = await self._generate_chart_from_analysis(analysis_result, file_data)
                ...
                },
                {
                    ...
                    "content": json.dumps({
                        "data_summary": {
                            ...
                            "columns": [col["name"] for col in file_source.get("schema", {}).get("columns", [])],
    ...
    async def _analyze_file_data(self, query: str, data: List[Dict], file_source: Dict) -> Dict:
    ...
    async def _generate_chart_from_analysis(self, analysis: Dict, data: List[Dict]) -> Dict:
    ...
    def _determine_chart_type(self, query: str, data: List[Dict]) -> str:
        ...
        # Time-based analysis
        if any(word in query_lower for word in ["trend", "over time", "growth", "change", "evolution"]):
            return "line"
        
        # Comparison analysis
        if any(word in query_lower for word in ["compare", "vs", "versus", "difference", "ranking"]):
            return "bar"
        
        # Distribution analysis
        if any(word in query_lower for word in ["distribution", "percentage", "proportion", "breakdown"]):
            return "pie"
        
        # Correlation analysis
        if any(word in query_lower for word in ["correlation", "relationship", "scatter", "pattern"]):
            return "scatter"
        
        # Cumulative/stacked analysis
        if any(word in query_lower for word in ["cumulative", "stacked", "area", "fill"]):
            return "area"
        
        # Multi-dimensional comparison
        if any(word in query_lower for word in ["dimensions", "factors", "criteria", "radar"]):
            return "radar"
        
        # Matrix/correlation matrix
        if any(word in query_lower for word in ["matrix", "heatmap", "correlation matrix", "grid"]):
            return "heatmap"
        
        # Default based on data characteristics
        if data and len(data[0]) >= 2:
            numeric_cols = [col for col in data[0].keys() if isinstance(data[0][col], (int, float))]
            categorical_cols = [col for col in data[0].keys() if not isinstance(data[0][col], (int, float))]
    ...
    def _generate_line_chart_config(self, data: List[Dict], analysis: Dict) -> Dict:
        ...
        for col in data[0].keys():
            if data:
                ...
                if isinstance(sample_value, (int, float)):
                    numeric_cols.append(col)
                elif isinstance(sample_value, str) and any(word in col.lower() for word in ["date", "time", "year", "month"]):
        ...
        for i, col1 in enumerate(numeric_cols):
            for j, col2 in enumerate(numeric_cols):
                if i <= j:  # Upper triangle
                    ...
                    value = data[0].get(col1, 0) if i == j else data[0].get(col1, 0) + data[0].get(col2, 0)
    ...
    def _generate_sample_data_from_schema(self, file_source: Dict) -> List[Dict]:
        ...
        try:
                ...
                for col in columns:
                    ...
                    if col_type in ["number", "integer", "float"]:
                        ...
                        if "min" in stats and "max" in stats:
                            import random
                            row[col_name] = round(random.uniform(stats["min"], stats["max"]), 2)
                        else:
                            row[col_name] = i + 1
            ...
            logger.info(f"📊 Generated {len(sample_data)} sample rows from schema for {file_source['name']}")
            return sample_data
    ...
    async def _get_file_context(self, source: Dict) -> Dict:
        ...
        try:
            ...
            file_context = {
                ...
                "file_type": source["name"].split(".")[-1].lower() if "." in source["name"] else "unknown",
                "schema": schema_data,
                "analysis_capabilities": {
    ...
    async def _get_database_context(self, source: Dict) -> Dict:
        ...
        try:
            ...
            db_context = {
                ...
                "schema_summary": self._build_database_schema_summary(source, schema_data)
            }
    ...
    async def _get_cube_context(self, source: Dict) -> Dict:
        ...
        try:
            ...
            cube_context = {
                ...
                "dimensions": self._extract_cube_dimensions(schema_data.get("cubes", [])),
                "segments": self._extract_cube_segments(schema_data.get("cubes", [])),
                "pre_aggregations": self._extract_pre_aggregations(schema_data.get("cubes", [])),
                "analysis_capabilities": {
                    "can_aggregate": True,
                    "can_dimension": True,
                    "can_filter": True,
                    ...
                    "total_measures": len(self._extract_cube_measures(schema_data.get("cubes", []))),
                    "total_dimensions": len(self._extract_cube_dimensions(schema_data.get("cubes", []))),
                    "supported_operations": ["semantic_queries", "pre_aggregations", "time_granularity", "business_logic"]
                },
    ...
    async def _fetch_database_schema(self, source_id: str) -> Dict:
        ...
        try:
            import httpx
            async with httpx.AsyncClient() as client:
                response = await client.get(f"http://localhost:8000/data/sources/{source_id}/schema")
    ...
    def _generate_database_query_templates(self, schema_data: Dict) -> List[str]:
        ...
        for table_name, table_info in tables.items():
            if table_info.get("columns"):
                ...
                templates.append(f"SELECT {', '.join(columns[:3])} FROM {table_name} LIMIT 10")
    ...
    def _generate_cube_query_templates(self, schema_data: Dict) -> List[str]:
        ...
        for cube in cubes[:3]:  # Max 3 cubes
            if cube.get("measures") and cube.get("dimensions"):
                ...
                dimension = cube["dimensions"][0]["name"] if cube["dimensions"] else "id"
                templates.append(f"{{ measures: ['{measure}'], dimensions: ['{dimension}'] }}")
    ...
    def _build_context_summary(self, context: Dict) -> str:
        ...
        if context["cube_js_sources"]:
            ...
            summary_parts.append(f"📊 {cube_count} Cube.js semantic models with {measure_count} metrics and {dimension_count} dimensions")
        
        if context["database_sources"]:
            db_count = len(context["database_sources"])
            table_count = len(context["table_schemas"])
            summary_parts.append(f"🗄️ {db_count} database connections with {table_count} tables")
    ...
    def _format_context_for_prompt(self, context: Dict) -> str:
        ...
        if context["cube_js_sources"]:
            ...
            for source in context["cube_js_sources"]:
                if "error" not in source:
                    prompt_parts.append(f"- {source['name']}: {len(source.get('cubes', []))} cubes")
                    if source.get("measures"):
                        prompt_parts.append(f"  Measures: {', '.join([m.get('title', m.get('name', '')) for m in source['measures'][:5]])}")
                    if source.get("dimensions"):
                        prompt_parts.append(f"  Dimensions: {', '.join([d.get('title', d.get('name', '')) for d in source['dimensions'][:5]])}")
        ...
        if context["database_sources"]:
            ...
            for source in context["database_sources"]:
                if "error" not in source:
                    prompt_parts.append(f"- {source['name']} ({source.get('type', 'unknown')})")
                    tables = source.get("tables", {})
                    if tables:
                        prompt_parts.append(f"  Tables: {', '.join(list(tables.keys())[:5])}")
    ...
    def _get_current_model(self) -> str:
        ...
        try:
            model_info = self.litellm_service.get_model_info()
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    ...
                    json={"sql": sql, "source_id": context.get("database_sources", [{}])[0].get("id")}
                )
                return response.json()
        except Exception as e:
    ...
    def _extract_cube_query(self, ai_response: str, context: Dict) -> Dict:
        ...
        try:
            ...
            if "measures" in ai_response.lower():
                ...
                measures = [m["name"] for m in context.get("available_metrics", []) 
                          if m["name"].lower() in ai_response.lower()]
                
                # Extract dimensions mentioned
                dimensions = [d["name"] for d in context.get("available_dimensions", []) 
                ...
                # Default query
                return {
                    "measures": [context.get("available_metrics", [{}])[0].get("name", "count")],
                    "dimensions": [],
                    "timeDimensions": [],
                    "filters": []
                }
            ...
            return {"measures": ["count"], "dimensions": [], "timeDimensions": [], "filters": []}
    ...
    async def _validate_analysis_result(self, result: Dict, context: Dict) -> Dict:
        ...
        result["validation"] = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "data_source_count": len(context.get("cube_js_sources", [])) + len(context.get("database_sources", [])),
    ...
    def _calculate_confidence_score(self, result: Dict, context: Dict) -> float:
        ...
        source_count = len(context.get("cube_js_sources", [])) + len(context.get("database_sources", []))
        if source_count > 1:
            base_score += 0.1
...
Provide a structured analysis focusing on data understanding aspects."""
            ...
            response = await self.litellm_service.generate_completion(
                messages=[
                    ...
                    {"role": "user", "content": "Analyze this data context and provide understanding insights."}
                ],
                max_tokens=800,
                temperature=0.3
            )
            ...
            return {
                ...
                'data_structure_summary': self._extract_data_structure_summary(data_context),
...
Create a comprehensive analysis plan."""
            ...
            response = await self.litellm_service.generate_completion(
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": "Create an analysis plan for this query."}
                ],
                max_tokens=600,
                temperature=0.4
            )
            ...
            return {
                ...
                'objectives': self._extract_analysis_objectives(response.get('content', '')),
...
Provide the SQL queries in code blocks with explanations."""
            ...
            response = await self.litellm_service.generate_completion(
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": "Generate SQL queries for this analysis."}
                ],
                max_tokens=1000,
                temperature=0.2
            )
...
Provide specific chart recommendations with ECharts options."""
            ...
            chart_recommendations = self._extract_chart_recommendations(response.get('content', ''))
...
Focus on practical business value."""
            ...
            response = await self.litellm_service.generate_completion(
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": "Generate business insights for this analysis."}
                ],
                max_tokens=1000,
                temperature=0.4
            )
...
Focus on business value and strategic impact."""
            ...
            response = await self.litellm_service.generate_completion(
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": "Provide business context for these insights."}
                ],
                max_tokens=800,
                temperature=0.4
            )
            ...
            return {
                ...
                'strategic_actions': self._extract_strategic_actions(response.get('content', ''))
            }
            
        except Exception as e:
            logger.error(f"Business context agent failed: {str(e)}")
            return {'business_context': 'Unable to provide business context', 'error': str(e)}
...
Structure the response logically and ensure all components are properly integrated."""
            ...
            response = await self.litellm_service.generate_completion(
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": "Synthesize all analysis components into a coherent response."}
                ],
                max_tokens=1500,
                temperature=0.3
            )
    ...
    async def _fallback_analysis(self, query: str, context: Dict, error: str) -> Dict[str, Any]:
    ...
    def _extract_key_findings(self, content: str) -> List[str]:
        ...
        lines = content.split('\n')
        for line in lines:
            if any(keyword in line.lower() for keyword in ['finding', 'discovery', 'pattern', 'trend']):
    ...
    def _extract_stakeholders(self, content: str) -> List[str]:
        ...
        lines = content.split('\n')
        for line in lines:
            if 'stakeholder' in line.lower() or 'team' in line.lower() or 'department' in line.lower():
    ...
    def _extract_strategic_actions(self, content: str) -> List[str]:
        ...
        lines = content.split('\n')
        for line in lines:
            if any(keyword in line.lower() for keyword in ['action', 'strategy', 'plan', 'initiative']):
    ...
    def _build_cube_schema_summary(self, cubes: List[Dict]) -> str:
        ...
        for cube in cubes:
            ...
            summaries.append(f"{cube_name}: {measures_count} measures, {dimensions_count} dimensions")
        
        return " | ".join(summaries)

    def _generate_cube_query_templates(self, measures: List[Dict], dimensions: List[Dict]) -> List[Dict]:
        """Generate example query templates for AI to use"""
        templates = []
        
        # Time series analysis template
        time_dimensions = [d for d in dimensions if "time" in d.get("type", "").lower() or "date" in d.get("name", "").lower()]
    ...
    def _get_supported_chart_types(self, context: Dict) -> List[str]:
        ...
        if context["cube_js_sources"]:
            chart_types.extend(["line", "bar", "pie", "scatter", "area", "heatmap", "table"])
        if context["database_sources"]:
            chart_types.extend(["line", "bar", "pie", "scatter", "area", "table", "dashboard"])
        if context["file_sources"]:
    ...
    def _get_query_capabilities(self, context: Dict) -> Dict:
        ...
        if context["cube_js_sources"]:
            capabilities["cube_js"] = {
                ...
                "advantages": ["Business-friendly", "Pre-aggregated", "Semantic modeling", "Performance optimized"]
            }
        ...
        if context["database_sources"]:
            capabilities["database"] = {
                ...
                "advantages": ["Full SQL power", "Complex joins", "Custom logic", "Real-time data"]
            }
        ...
        if context["file_sources"]:
            capabilities["file"] = {
                ...
                "advantages": ["Simple structure", "Quick analysis", "No setup required", "Direct access"]
            }
    ...
    def _build_database_schema_summary(self, schema_data: Dict) -> str:
        ...
        summary = f"Database with {len(tables)} tables across {len(schemas)} schemas\n\n"
        ...
        for table_name, table_info in tables.items():
            ...
            numeric_cols = [col for col in columns if col.get("data_type") in ["integer", "numeric", "decimal", "float"]]
            text_cols = [col for col in columns if col.get("data_type") in ["varchar", "text", "string"]]
            date_cols = [col for col in columns if col.get("data_type") in ["timestamp", "date", "time"]]
    ...
    def _generate_file_query_templates(self, schema_data: Dict) -> List[Dict]:
        ...
        numeric_cols = [col for col in columns if col.get("type") in ["number", "integer", "float"]]
    ...
    def _build_file_schema_summary(self, source: Dict, schema_data: Dict) -> str:
        ...
        numeric_cols = [col for col in columns if col.get("type") in ["number", "integer", "float"]]
        text_cols = [col for col in columns if col.get("type") == "string"]
        date_cols = [col for col in columns if col.get("type") in ["date", "time", "datetime"]]
    ...
    def _has_time_columns_file(self, schema_data: Dict) -> bool:
        ...
        for col in columns:
            ...
            if any(time_word in col_type for time_word in ["date", "time", "datetime"]) or \
               any(time_word in col_name for time_word in ["date", "time", "created", "updated"]):
                return True
        return False

...
class FunctionCallingService:
    ...
    def __init__(self):
        self.function_calling_enabled = os.getenv('ENABLE_FUNCTION_CALLING', 'true').lower() == 'true'
    ...
    async def process_with_function_calling(
        ...
        try:
            ...
            chart_type = 'bar'
            if chart_recommendation['success'] and chart_recommendation['recommendations']:
            ...
            insights = await self._generate_business_insights(
                data, 
                chart_type, 
                context.get('business_domain') if context else None
            )
            ...
            return {
                ...
                'business_insights': insights.get('insights', []) if insights['success'] else [],
    ...
    async def _generate_echarts_config(
        ...
        try:
            ...
            if not y_axis_field and data:
                ...
                y_axis_field = numeric_fields[0] if numeric_fields else list(data[0].keys())[1]
            ...
            if chart_type in ['line', 'bar', 'area']:
                ...
                config.update({
                    ...
                    'series': [{
                        'name': y_axis_field.replace('_', ' ').title() if y_axis_field else 'Value',
                        'type': chart_type,
                        'data': values
                    }]
                })
    ...
    async def _recommend_chart_type(
        ...
        try:
            ...
            if categorical_count > 0 and numeric_count > 0:
                recommendations.append({'type': 'bar', 'confidence': 0.8, 'reason': 'Categorical comparison'})
            if numeric_count >= 2:
                recommendations.append({'type': 'scatter', 'confidence': 0.6, 'reason': 'Multiple numeric variables'})
            if categorical_count > 0 and numeric_count == 1:
                recommendations.append({'type': 'pie', 'confidence': 0.7, 'reason': 'Single numeric with categories'})
            ...
            return {
                ...
                'primary_recommendation': recommendations[0] if recommendations else {'type': 'bar', 'confidence': 0.5}
            }

...
class LiteLLMService:
    ...
    def __init__(self):
        ...
        if self.azure_api_key and self.azure_endpoint:
            ...
            logger.warning(
                "⚠️ No AI models configured - service will use fallback responses"
            )
    ...
    def _validate_azure_config(self):
        ...
        if self.azure_api_key and self.azure_endpoint:
            ...
            logger.info(
                f"   - API Key: {'*' * 8 + self.azure_api_key[-4:] if self.azure_api_key else 'Not set'}"
            )
            ...
            logger.warning(
                f"   - API Key: {'Set' if self.azure_api_key else 'Not set'}"
            )
            logger.warning(
                f"   - Endpoint: {'Set' if self.azure_endpoint else 'Not set'}"
            )

        if self.openai_api_key:
            logger.info("✅ OpenAI Configuration:")
            logger.info(
                f"   - API Key: {'*' * 8 + self.openai_api_key[-4:] if self.openai_api_key else 'Not set'}"
            )
...
"""
            ...
            model_config = self.available_models.get(
                selected_model, self.available_models[self.default_model]
            )
            ...
            litellm_params = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are an expert data analyst. Analyze queries and return structured JSON responses.",
                    },
            ...
            if "gpt-5" in model_config["model"]:
                litellm_params["max_completion_tokens"] = min(
                    1000, model_config["max_tokens"]
                )
            else:
            ...
            logger.info(
                f"✅ Query analysis completed: {analysis.get('query_type', [])}"
            )
            return analysis
...
"""
            ...
            selected_model = (
                "azure_gpt5_mini" if self.azure_api_key else "openai_gpt4_mini"
            )
            model_config = self.available_models.get(
                selected_model, self.available_models[self.default_model]
            )
            ...
            response = await acompletion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert data visualization specialist. Recommend optimal chart types based on data characteristics.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.2,
            ...
            logger.info(
                f"✅ Chart recommendations generated: {recommendations.get('primary_recommendation')}"
            )
...
"""
            ...
            response = await acompletion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a business intelligence expert. Generate actionable insights from data analysis.",
                    },
                    {"role": "user", "content": prompt},
                ],
                max_tokens=1500,
    ...
    def _fallback_chart_recommendations(
        self, data_analysis: Dict, query_analysis: Dict
    ) -> Dict[str, Any]:
    ...
    def _fallback_business_insights(
        self, data: List[Dict], query_analysis: Dict
    ) -> List[Dict[str, Any]]:
    ...
    async def generate_completion(
        ...
        try:
            ...
            logger.info(
                f"🔍 First choice: {response.choices[0] if response.choices else 'None'}"
            )
...
Please try connecting a data source and I'll be able to provide more specific assistance!"""
        ...
        # Check for general help queries
            ...
            word in prompt_lower
            for word in ["help", "how", "what", "guide", "tutorial", "start", "begin"]
        ):
...
Would you like me to help you connect a data source or explain any specific features?"""
        ...
        # Check for specific technical questions
            ...
            word in prompt_lower
            for word in ["sql", "query", "database", "connection", "api"]
        ):
...
Please try connecting a data source to get started, or let me know if you need help with any specific features!"""
    ...
    def _validate_and_clean_response(self, content: str, original_prompt: str) -> str:
        ...
        if not content or content.strip() == "":
            ...
            return self._generate_fallback_response(
                original_prompt, Exception("Empty response")
            )
        ...
        if len(cleaned) < 10:
            ...
            return self._generate_fallback_response(
                original_prompt, Exception("Response too short")
            )
        ...
        if any(indicator in cleaned.lower() for indicator in error_indicators):
            ...
            return self._generate_fallback_response(
                original_prompt, Exception("Response contains errors")
            )
        ...
        try:
            ...
            test_params = {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": "Hello, respond with 'Azure OpenAI connection successful'",
                    }
                ],
                "temperature": 1.0,  # GPT-5 only supports temperature=1
                "api_key": self.azure_api_key,
    ...
    async def analyze_data_with_ai(
        self, query: str, data_summary: Dict[str, Any], data_source_id: str
    ) -> Dict[str, Any]:
        """Enhanced AI data analysis with multiple specialized agents"""
        try:
            logger.info(
                f"🧠 Starting enhanced AI data analysis for query: {query[:50]}..."
            )
            ...
            **Example Response Structure:**
            ...
            response = await acompletion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert data analyst and business intelligence specialist. Provide comprehensive, actionable insights.",
                    },
    ...
    def _extract_chart_recommendations(self, content: str) -> List[str]:
        ...
        return (
            recommendations if recommendations else ["bar", "line"]
    ...
    async def generate_completion_stream(
        ...
        try:
            ...
            enhanced_messages = self._enhance_messages_with_context(
                messages, data_context
            )
            ...
            logger.info(
                f"🚀 Starting streaming completion with model: {model_config['model']}"
            )
            ...
            chart_recommendations = await self._generate_chart_recommendations(
                content, data_context
            )

            # Generate SQL queries if data analysis is involved
            sql_queries = []
            if data_context and data_context.get("type") in [
                "database",
                "warehouse",
                "cube",
            ]:
    ...
    def _enhance_messages_with_context(
        self, messages: List[Dict[str, str]], data_context: Dict[str, Any]
    ) -> List[Dict[str, str]]:
        ...
        for message in messages:
            if message["role"] == "system" and not system_message_added:
                ...
                enhanced_messages.append({"role": "system", "content": context_message})
                system_message_added = True
            else:
                enhanced_messages.append(message)

        if not system_message_added:
            enhanced_messages.insert(0, {"role": "system", "content": context_message})
    ...
    def _build_context_message(self, data_context: Dict[str, Any]) -> str:
        ...
        if data_context.get("analysis_capabilities"):
            ...
            context_parts.append(
                f"Capabilities: {', '.join([k for k, v in caps.items() if v])}"
            )
        ...
        if data_context.get("schema"):
            ...
            if schema.get("tables"):
                context_parts.append(
                    f"Tables: {len(schema['tables'])} tables available"
                )
                for table in schema["tables"][:3]:  # Show first 3 tables
                    context_parts.append(
                        f"  - {table.get('name', 'Unknown')}: {table.get('rowCount', 0)} rows"
                    )
        ...
        if data_context.get("cube_schema"):
            ...
            if cube.get("cubes"):
                for cube_info in cube["cubes"][:2]:  # Show first 2 cubes
                    context_parts.append(
                        f"  - {cube_info.get('name', 'Unknown')}: {len(cube_info.get('dimensions', []))}D, {len(cube_info.get('measures', []))}M"
                    )
        ...
        if data_context.get("file_info"):
            ...
            context_parts.append(
                f"File: {file_info.get('filename', 'Unknown')} ({file_info.get('row_count', 0)} rows, {len(file_info.get('columns', []))} columns)"
            )
        ...
        if data_context.get("user_context", {}).get("analysis_preferences"):
            ...
            context_parts.append(
                f"User Preferences: {prefs.get('analysis_depth', 'standard')} analysis, include charts: {prefs.get('include_charts', True)}"
            )
...
Focus on accuracy, relevance, and practical value for business users."""
    ...
    def _extract_sql_queries(
        self, content: str, data_context: Dict[str, Any]
    ) -> List[str]:

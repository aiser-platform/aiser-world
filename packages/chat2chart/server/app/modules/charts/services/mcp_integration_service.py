...
class MCPIntegrationService:
    ...
    def __init__(self):
        self.mcp_server_url = os.getenv('MCP_ECHARTS_SERVER_URL', 'http://localhost:3001')
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.mcp_server_url}/health") as response:
                    if response.status == 200:
                        ...
                        logger.info(f"✅ Connected to MCP ECharts server")
                        return {
                            'success': True,
                            'connected': True,
    ...
    async def generate_chart_with_mcp(
        ...
        try:
            ...
            logger.info(f"📊 Generating chart with MCP ECharts")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        raise Exception(f"MCP generation failed: HTTP {response.status} - {error_text}")
    ...
    async def get_chart_recommendations_from_mcp(
        ...
        try:
            ...
            logger.info(f"💡 Getting chart recommendations from MCP")
    ...
    def _fallback_recommendations(self, data_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Fallback recommendations when MCP is not available"""
        logger.info("💡 Using fallback recommendations")
        
        # Simple rule-based recommendations
        numeric_cols = len(data_analysis.get('statistical_analysis', {}).get('numeric_columns', []))
        categorical_cols = len(data_analysis.get('statistical_analysis', {}).get('categorical_columns', []))

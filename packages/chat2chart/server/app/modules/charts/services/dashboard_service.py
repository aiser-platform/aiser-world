...
class DashboardService:
    ...
    async def list_dashboards(
        ...
        try:
            logger.info(
                f"📊 Listing dashboards for project {project_id}, user {user_id}"
            )
            ...
            query = select(Dashboard).where(
                and_(
                    ...
                    or_(Dashboard.created_by == user_id, Dashboard.is_public),
                )
            )
            ...
            query = (
                query.offset(offset).limit(limit).order_by(Dashboard.updated_at.desc())
            )
            ...
            for dashboard in dashboards:
                dashboard_data = {
                    ...
                    "last_viewed_at": dashboard.last_viewed_at.isoformat()
    ...
    async def create_dashboard(
        self, dashboard_data: DashboardCreateSchema, user_id: int
    ) -> Dict[str, Any]:
    ...
    async def update_dashboard(
        self, dashboard_id: str, dashboard_data: DashboardUpdateSchema, user_id: int
    ) -> Dict[str, Any]:

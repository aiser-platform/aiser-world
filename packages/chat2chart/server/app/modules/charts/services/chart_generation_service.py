...
class ChartGenerationService:
    ...
    async def generate_chart_from_query(
        ...
        try:
            logger.info(
                f"🎨 Generating chart for query: {query_analysis.get('original_query', 'Unknown')}"
            )
            ...
            cube_data = {"data": data, "annotation": self._generate_annotation(data)}
            ...
            return {"success": False, "error": str(error), "fallback_available": True}
    ...
    async def generate_chart_from_file_data(
        ...
        try:
            logger.info(
                f"📁 Generating chart from file data: {file_metadata.get('name', 'Unknown')}"
            )
    ...
    async def get_chart_recommendations(
        ...
        try:
            ...
            for chart_type in chart_types:
                if self.mcp_echarts._is_chart_type_compatible(
                    chart_type, data_analysis
                ):
                    score = self._calculate_chart_type_score(
                        chart_type, data_analysis, query_analysis
                    )
    ...
    def _generate_annotation(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for key in first_row.keys():
            if any(
                suffix in key for suffix in [".count", ".sum", ".avg", ".max", ".min"]
            ):
                ...
                suffix in key for suffix in [".day", ".month", ".year", ".hour", "Date"]
            ):
    ...
    def _infer_query_type_from_text(self, text: str) -> List[str]:
        ...
        if any(
            word in text_lower for word in ["trend", "over time", "growth", "change"]
        ):
            query_types.append("trends")
        if any(
            word in text_lower for word in ["compare", "vs", "versus", "difference"]
        ):
    ...
    def _calculate_chart_type_score(
        ...
        if patterns.get("has_trend") and chart_type == "line":
            score += 1.0
        if (
            patterns.get("correlation")
            and abs(patterns["correlation"]) > 0.7
    ...
    def _get_recommendation_reason(
        self, chart_type: str, data_analysis: Dict[str, Any]
    ) -> str:

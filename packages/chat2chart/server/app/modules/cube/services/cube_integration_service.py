...
class CubeIntegrationService:
    ...
    async def generate_cube_schema(
        ...
        try:
            logger.info(f"🧠 Generating Cube.js schema for data source: {data_source.get('name')}")
            ...
            if not ai_response.get('success'):
                raise Exception(f"AI schema generation failed: {ai_response.get('error')}")
            ...
            return {
                'success': False,
                'error': str(e),
                'fallback_schema': self._generate_fallback_schema(data_source, sample_data)
            }
    
    async def _analyze_data_structure(self, sample_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for column in df.columns:
            ...
            if pd.api.types.is_numeric_dtype(col_data):
                if col_data.nunique() / len(col_data) > 0.8:  # High cardinality = measure
                    analysis['measures'].append({
                        'name': column,
                        'type': 'sum' if col_data.dtype in ['int64', 'float64'] else 'count',
                        'sql': column
                    })
                else:
    ...
    async def generate_chart_from_query(
        ...
        try:
            ...
            query_analysis = await self.ai_service.analyze_natural_language_query(
                natural_query,
                context={'available_cubes': available_cubes}
            )
            
            # Generate Cube.js query
            cube_query = await self._generate_cube_query(query_analysis, available_cubes)

...
class YAMLSchemaService:
    ...
    async def generate_yaml_schema(
        ...
        try:
            logger.info(f"📋 Generating YAML schema for {data_source_type} data source: {data_source_id}")
            
            # Get base template
            base_template = self.schema_templates.get(data_source_type, self.schema_templates['database'])
            
            # Generate schema based on data source type
            if data_source_type == 'database':
                yaml_schema = await self._generate_database_yaml_schema(data_source_id, raw_schema, user_preferences)
            elif data_source_type == 'file':
                yaml_schema = await self._generate_file_yaml_schema(data_source_id, raw_schema, user_preferences)
            elif data_source_type == 'api':
                yaml_schema = await self._generate_api_yaml_schema(data_source_id, raw_schema, user_preferences)
            elif data_source_type == 'warehouse':
                yaml_schema = await self._generate_warehouse_yaml_schema(data_source_id, raw_schema, user_preferences)
            else:
                yaml_schema = await self._generate_generic_yaml_schema(data_source_id, raw_schema, user_preferences)
    ...
    async def validate_yaml_schema(self, schema_content: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, rel in enumerate(relationships):
                relationship_errors.extend(self._validate_relationship_structure(rel, i))
    ...
    async def update_schema_from_verification(
        ...
        try:
            ...
            # Apply user feedback
            updated_schema = await self._apply_user_feedback(current_schema, user_feedback)
    ...
    async def export_schema(self, data_source_id: str, format: str = 'yaml') -> Dict[str, Any]:
        ...
        try:
            ...
            if format.lower() == 'yaml':
                export_content = yaml.dump(schema, default_flow_style=False, sort_keys=False)
    ...
    def _is_measure_column(self, column: Dict[str, Any]) -> bool:
        ...
        return column_type in ['int', 'integer', 'bigint', 'smallint', 'decimal', 'numeric', 'float', 'double', 'real']
    ...
    def _is_time_dimension_column(self, column: Dict[str, Any]) -> bool:
        ...
        return (column_type in time_types or 
                any(keyword in column_name for keyword in time_keywords))
    
    async def _generate_relationships(self, tables: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            for column in columns:
                if column.get('foreign_key'):
                    relationships.append({
                        ...
                        'to_column': column['foreign_key'].split('.')[1] if '.' in column['foreign_key'] else 'id',
                        'description': f"Foreign key relationship from {table_name}.{column['name']}"
                    })
        
        return relationships
    
    async def _save_yaml_schema(self, schema: Dict[str, Any], data_source_id: str) -> Path:
    ...
    def _validate_relationship_structure(self, relationship: Dict[str, Any], index: int) -> List[str]:
        """Validate relationship structure"""
        errors = []
        
        required_fields = ['name', 'type', 'from_table', 'from_column', 'to_table', 'to_column']
    ...
    def _generate_verification_checklist(self, schema: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if relationships:
            checklist.append({
                ...
                'items': [
                    {'item': 'Foreign key relationships are correct', 'required': True},
                    {'item': 'Relationship names are meaningful', 'required': False}
                ]
            })
        
        return checklist
    
    async def _generate_schema_suggestions(self, schema: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for table in tables:
            ...
            time_columns = [col for col in columns if self._is_time_dimension_column(col)]
            if not time_columns:
                suggestions.append({
                    'type': 'info',
                    'table': table_name,
        ...
        preview = {
            ...
            'summary': {
                ...
                'estimated_measures': sum(
                    len([col for col in table.get('columns', []) if self._is_measure_column(col)])
                    for table in tables
                ),
                'estimated_dimensions': sum(
                    len([col for col in table.get('columns', []) if self._is_dimension_column(col)])
                    for table in tables
                ),
                'estimated_time_dimensions': sum(
                    len([col for col in table.get('columns', []) if self._is_time_dimension_column(col)])
                    for table in tables
                )
            },
            'tables_preview': []
    ...
    async def _apply_user_feedback(self, schema: Dict[str, Any], user_feedback: Dict[str, Any]) -> Dict[str, Any]:
                            ...
                            for column in table.get('columns', []):
                                if column.get('name') == column_name:
                                    ...
                                    if 'cube_settings' in change:
                                        column['cube_settings'] = change['cube_settings']
                                    break
    ...
    async def _generate_cube_schema_from_yaml(self, yaml_schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for cube in cube_schema.get('cubes', []):
            ...
            if cube.get('preAggregations'):
                ...
                for agg_name, agg_config in cube['preAggregations'].items():
                    ...
                    js_lines.append(f"      measures: {json.dumps(agg_config['measures'])},")
                    js_lines.append(f"      dimensions: {json.dumps(agg_config['dimensions'])},")

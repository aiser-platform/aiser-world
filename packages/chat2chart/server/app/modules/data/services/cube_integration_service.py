...
class CubeIntegrationService:
    ...
    async def create_cube_schema(self, data_source: Dict[str, Any], schema_type: str = 'star') -> Dict[str, Any]:
    ...
    async def _analyze_data_structure(self, data_source: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if data_source['type'] == 'file':
                ...
                for col_name, col_info in columns.items():
                    ...
                    col_info['null_percentage'] = (col_info['null_count'] / len(sample_data)) * 100
                    col_info['cardinality'] = 'high' if col_info['unique_count'] > len(sample_data) * 0.8 else 'low'
                ...
                for col_name, col_info in columns.items():
                    if col_info['type'] in ['string', 'date', 'boolean'] or col_info['cardinality'] == 'low':
                        dimensions.append({
                            'name': col_name,
                            'type': col_info['type'],
    ...
    async def _generate_star_schema(self, analysis: Dict[str, Any], data_source: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for measure in analysis.get('measures', []):
                fact_table['measures'].append({
                    'name': measure['name'],
                    'sql': measure['name'],
                    'type': 'sum' if measure['type'] in ['number', 'integer'] else 'count',
    ...
    async def _generate_snowflake_schema(self, analysis: Dict[str, Any], data_source: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _generate_flat_schema(self, analysis: Dict[str, Any], data_source: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _apply_performance_optimizations(self, schema: Dict[str, Any]) -> Dict[str, Any]:
                ...
                if self.pre_aggregation_enabled:
                    cube['preAggregations'] = {
                        'main': {
                            ...
                            'timeDimension': self._find_time_dimension(cube.get('dimensions', [])),
                            'granularity': 'day'
                        }
                    }
    ...
    async def _apply_security_measures(self, schema: Dict[str, Any], data_source: Dict[str, Any]) -> Dict[str, Any]:
        """Apply security measures to the schema"""
        # Implementation details...
        return schema

    async def _save_schema_to_file(self, schema: Dict[str, Any], data_source_name: str) -> Path:
        """Save schema to file"""
        # Implementation details...
        return Path(f"{data_source_name}_schema.yml")

    async def deploy_cube_schema(self, schema: Dict[str, Any], data_source: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _deploy_to_cube_server(self, schema: Dict[str, Any], data_source: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for cube_name, cube_config in schema.get('cubes', {}).items():
                ...
                if 'data_source' in cube_config:
                    cube_js_code.append(f"  sql: `SELECT * FROM {cube_config['data_source']}`,")
                ...
                if 'dimensions' in cube_config:
                    ...
                    for dim in cube_config['dimensions']:
                        ...
                        cube_js_code.append(f"      sql: `${{{dim['sql_column'] or dim['name']}}}`,")
                        cube_js_code.append("    },")
                    cube_js_code.append("  },")
                ...
                if 'measures' in cube_config:
                    ...
                    for measure in cube_config['measures']:
                        ...
                        cube_js_code.append(f"      type: '{measure.get('type', 'sum')}',")
                        cube_js_code.append(f"      sql: `${{{measure['sql_column'] or measure['name']}}}`,")
                        cube_js_code.append("    },")
                    cube_js_code.append("  },")
    ...
    async def _get_cube_metadata(self) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(self.cube_meta_endpoint, headers=headers) as response:
                    if response.status == 200:
                        return await response.json()
                    else:
                        logger.warning(f"⚠️ Failed to get metadata: HTTP {response.status}")
                        return {'cubes': []}
        except Exception as e:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        raise Exception(f"Deployment failed: HTTP {response.status} - {error_text}")
    ...
    async def execute_cube_query(self, query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                # Execute query
                async with session.post(
                    f"{self.cube_server_url}/v1/load",
                    headers=headers,
                    json=query
                ) as response:
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        raise Exception(f"Query execution failed: HTTP {response.status} - {error_text}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.cube_server_url}/meta") as response:
                    if response.status == 200:
                        ...
                        raise Exception(f"Failed to get metadata: HTTP {response.status}")
        except Exception as e:
            logger.error(f"❌ Failed to get Cube.js metadata: {str(e)}")
            return {'error': str(e)}
    ...
    async def get_cube_data_preview(self, cube_name: str, limit: int = 10) -> Dict[str, Any]:

...
class CubeDataModelingService:
    ...
    def __init__(self):
        self.cube_api_url = os.getenv('CUBE_API_URL', 'http://cube-server:4000/cubejs-api/v1')
    ...
    async def analyze_data_source(
        ...
        try:
            ...
            cube_schema = await self._generate_cube_schema(data_source_id, analysis, connection_info)
            
            # Generate visual modeling flow
            visual_flow = self._generate_visual_modeling_flow(analysis)
            
            # Generate YAML schema
            yaml_schema = self._generate_yaml_schema(data_source_id, analysis, connection_info)
    ...
    def _analyze_data_structure(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for column in df.columns:
            ...
            elif 'date' in column.lower() or 'time' in column.lower() or 'created' in column.lower():
    ...
    async def _generate_cube_schema(
        ...
        if connection_info:
            ...
            data_source = {
                'type': 'file',
                'source': f"uploaded_data_{data_source_id}"
            }
    ...
    def _generate_visual_modeling_flow(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_yaml_schema(
        ...
        cube_config = {
            'cubes': [
                {
                    'name': cube_name,
                    'sql_table': connection_info.get('table', data_source_id) if connection_info else data_source_id,
                    'dimensions': {},
                    'measures': {}
                }
            ]
    ...
    def _get_modeling_types(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Star Schema
        if len(analysis.get('dimensions', [])) > 2 and len(analysis.get('measures', [])) > 0:
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        null_columns = [
            col for col, info in analysis.get('columns', {}).items() 
    ...
    async def _test_warehouse_connection(self, config: Dict[str, Any]) -> Dict[str, Any]:

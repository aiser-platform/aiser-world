...
class RealCubeIntegrationService:
    ...
    async def create_database_connection(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            connection_id = f"{db_type}_{config.get('database')}_{int(datetime.now().timestamp())}"
            connection_metadata = {
                'id': connection_id,
                'type': db_type,
                'config': config,
    ...
    async def create_cube_schema(self, connection_id: str, schema_config: Dict[str, Any]) -> Dict[str, Any]:
        """Create real Cube.js schema from database connection"""
        try:
            logger.info(f"📋 Creating real Cube.js schema for connection: {connection_id}")
            ...
            if deployment_result['success']:
                logger.info(f"✅ Real Cube.js schema created and deployed: {schema_file}")
                return {
                    'success': True,
    ...
    async def _test_direct_connection(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if db_type == 'postgresql':
                ...
                conn_str = f"DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={config['host']},{config.get('port', driver_info['default_port'])};DATABASE={config['database']};UID={config['username']};PWD={config['password']}"
                # conn = pyodbc.connect(conn_str, timeout=10)  # Temporarily commented out
                raise NotImplementedError("SQL Server connections temporarily disabled due to missing ODBC libraries")
                conn.close()
    ...
    async def _create_cube_data_source_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Create Cube.js data source configuration"""
        db_type = config['type'].lower()
        driver_info = self.database_drivers[db_type]
    ...
    async def _save_connection_metadata(self, connection_metadata: Dict[str, Any]) -> None:
        ...
        try:
            ...
            async with get_async_session() as db:
                ...
                logger.info(f"✅ Connection metadata saved: {connection_metadata['id']}")
                
        except Exception as e:
            logger.error(f"❌ Failed to save connection metadata: {str(e)}")
    
    async def _get_connection_metadata(self, connection_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _generate_cube_schema_from_database(self, tables: List[Dict], config: Dict, schema_config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            for table in tables:
                ...
                for column in table['columns']:
                    ...
                    if column_type in ['int', 'integer', 'bigint', 'smallint', 'decimal', 'numeric', 'float', 'double', 'real']:
                        ...
                        dimensions.append({
                            'name': column_name,
                            'sql': column_name,
                            'type': 'string' if column_type not in ['date', 'datetime', 'timestamp'] else 'time',
                            'title': column_name.replace('_', ' ').title()
                        })
    ...
    async def _save_cube_schema_file(self, cube_schema: Dict[str, Any], connection_id: str) -> Path:
            ...
            if cube.get('preAggregations'):
                ...
                for agg_name, agg_config in cube['preAggregations'].items():
                    ...
                    if agg_config.get('timeDimension'):
                        js_lines.append(f"      timeDimension: '{agg_config['timeDimension']}',")
    ...
    async def _is_cube_running(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.cube_server_url}/health", timeout=aiohttp.ClientTimeout(total=5)) as response:
    ...
    async def _start_cube_server(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'success': True,
                'message': 'Cube.js server started successfully'
            }

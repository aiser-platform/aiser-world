...
# Business metrics
ACTIVE_USERS = Gauge(
    "active_users_total", "Total active users", ["organization_id"], registry=REGISTRY
)
...
class MetricsCollector:
    ...
    async def collect_system_metrics(self):
        ...
        try:
            ...
            ERRORS_TOTAL.labels(
                error_type="system_metrics", component="collector"
            ).inc()
    ...
    async def collect_business_metrics(self, session: AsyncSession):
        ...
        try:
            ...
            ERRORS_TOTAL.labels(
                error_type="business_metrics", component="collector"
            ).inc()
...
class AlertManager:
    ...
    async def _check_alert_rule(self, rule: AlertRule, session: AsyncSession):
        ...
        try:
            ...
            current_value = await self._get_metric_value(
                rule.metric_name, rule.labels, session
            )
    ...
    async def _get_metric_value(
        self, metric_name: str, labels: Dict[str, str], session: AsyncSession
    ) -> Optional[float]:
        ...
        try:
            ...
            query = (
                ...
                .where(
                    and_(
                        MetricRecord.name == metric_name, MetricRecord.labels == labels
                    )
                )
    ...
    async def _trigger_alert(
        self, rule: AlertRule, current_value: float, session: AsyncSession
    ):
        """Trigger an alert"""
        try:
            ...
            logger.warning(
                f"Alert triggered: {rule.name} - {rule.metric_name} {rule.condition} {rule.threshold} (current: {current_value})"
            )
...
class PerformanceMonitor:
    ...
    async def monitor_request(self, request: Request, call_next):
        ...
        try:
            ...
            # Record error
            ERRORS_TOTAL.labels(
                error_type=type(e).__name__, component="http_handler"
            ).inc()

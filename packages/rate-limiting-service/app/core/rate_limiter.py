...
class RateLimiter:
    ...
    async def _check_rate_limit_algorithm(
        ...
        f"{key}:{int(now // config.window_size)}"
        ...
        reset_time = datetime.fromtimestamp(
            (int(now // config.window_size) + 1) * config.window_size, tz=timezone.utc
        )
    ...
    async def _check_quota_limit(
        self, identifier: str, quota_type: QuotaType, config: RateLimitConfig
    ) -> Dict[str, Any]:
    ...
    async def record_usage(
        self, identifier: str, quota_type: QuotaType, amount: int = 1
    ):
...
class UsageTracker:
    ...
    async def _update_counters(
        self, organization_id: str, user_id: str, endpoint: str, data_size: int
    ):
    ...
    async def get_usage_stats(
        ...
        while current_date <= end_date:
            ...
            if api_calls:
                ...
                stats["daily_breakdown"][date_key] = {"api_calls": int(api_calls)}
            ...
            if data_transfer:
                ...
                if date_key in stats["daily_breakdown"]:
                    stats["daily_breakdown"][date_key]["data_transfer"] = int(
                        data_transfer
                    )
...
class QuotaManager:
    ...
    async def get_quota_usage(
        self, organization_id: str, quota_type: QuotaType
    ) -> Dict[str, Any]:
    ...
    async def check_quota_exceeded(
        self, organization_id: str, quota_type: QuotaType, amount: int = 1
    ) -> bool:
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    result = await rate_limiter.check_rate_limit(
        identifier, quota_type=QuotaType.API_CALLS
    )
